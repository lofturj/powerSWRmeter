...
void adc_init(void)
{ 
  //pinMode(Pfwd, INPUT);             // Defining the pins as inputs will cause a hysteresis error around 3.3V/2.
  //pinMode(Pref, INPUT);             // In any case, it is not necessary to define them, as the ADC function does it.
  #if ADCREF_BUILTIN
  ...
  adc->adc1->setAveraging(16);        // adc_low_speed.  This results in each measurement taking 80us
  
  adc_ref = ADC_RES;                      // ADC Reference voltage set for the Builtin + series R, unless later selected differently
}
...
void adc_poll_and_feed_circular(void)
{
  ...
  // Each poll of the builtin ADC is measured to take 130us if averaging set at 8.
  {
    ...
    {
      measure.fwd[measure.incount] = result.result_adc1;
      measure.rev[measure.incount] = result.result_adc0;
    }
    else  // error
    {
      measure.fwd[measure.incount] = -1;      // Should never happen
      measure.rev[measure.incount] = -1;
    }
    measure.incount++;                        // 8 bit value, rolls over at 256
  }
}
